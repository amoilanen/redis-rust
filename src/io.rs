...
pub fn read_single_message(stream: &mut TcpStream) -> Result<Option<protocol::DataType>, anyhow::Error> {
...
pub fn read_messages(stream: &mut TcpStream) -> Result<Vec<protocol::DataType>, anyhow::Error> {
    if let Some(message_bytes) = read_bytes(stream)? {
