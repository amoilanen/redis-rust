...
#[test]
fn test_rpush_returns_new_length_as_integer() {
    ...
    let resp = client
        .send_command(&["RPUSH", "fruits", "apple", "banana", "cherry"])
        .unwrap();
    assert_eq!(resp, "3");
}

#[test]
fn test_rpush_repeated_calls_grow_the_list() {
    let port = free_port();
    let server = ServerProcess::start_master(port);
    let mut client = server.client();

    assert_eq!(client.send_command(&["RPUSH", "k", "a"]).unwrap(), "1");
    assert_eq!(client.send_command(&["RPUSH", "k", "b"]).unwrap(), "2");
    assert_eq!(
        client.send_command(&["RPUSH", "k", "c", "d"]).unwrap(),
        "4"
    );

    // Read the full list back to confirm append order.
    let resp = client.send_command(&["LRANGE", "k", "0", "-1"]).unwrap();
...
#[test]
fn test_lpush_returns_new_length_as_integer() {
    let port = free_port();
    let server = ServerProcess::start_master(port);
    let mut client = server.client();

    let resp = client
        .send_command(&["LPUSH", "letters", "a", "b", "c"])
        .unwrap();
    assert_eq!(resp, "3");
}

#[test]
fn test_lpush_inserts_in_reverse_order() {
    ...
    client
        ...
        .unwrap();
    let resp = client
        .send_command(&["LRANGE", "letters", "0", "-1"])
        .unwrap();
...
#[test]
fn test_rpush_and_lpush_combined_produce_expected_order() {
    let port = free_port();
    let server = ServerProcess::start_master(port);
    let mut client = server.client();
    ...
    assert_eq!(
        client.send_command(&["RPUSH", "queue", "x", "y", "z"]).unwrap(),
        "3"
    );
    // ... then prepend two via LPUSH: "b" first ([b,x,y,z]), then "a" ([a,b,x,y,z]).
    assert_eq!(
        client.send_command(&["LPUSH", "queue", "b", "a"]).unwrap(),
        "5"
    );

    let resp = client.send_command(&["LRANGE", "queue", "0", "-1"]).unwrap();
...
#[test]
fn test_lrange_full_range_via_negative_one() {
    let port = free_port();
    let server = ServerProcess::start_master(port);
    let mut client = server.client();

    client
        .send_command(&["RPUSH", "nums", "one", "two", "three", "four", "five"])
        .unwrap();

    let resp = client.send_command(&["LRANGE", "nums", "0", "-1"]).unwrap();
    assert_eq!(resp, "one,two,three,four,five");
}

#[test]
fn test_lrange_negative_indices_count_from_end() {
    let port = free_port();
    let server = ServerProcess::start_master(port);
    let mut client = server.client();
    ...
    let resp = client.send_command(&["LRANGE", "nums", "1", "-2"]).unwrap();
    assert_eq!(resp, "two,three,four");
}

#[test]
fn test_lrange_clamps_out_of_range_stop() {
    let port = free_port();
    let server = ServerProcess::start_master(port);
    let mut client = server.client();

    client
        .send_command(&["RPUSH", "nums", "one", "two", "three"])
        .unwrap();

    // Stop index well past the end is clamped to the list length.
    let resp = client.send_command(&["LRANGE", "nums", "1", "100"]).unwrap();
    assert_eq!(resp, "two,three");
}

#[test]
fn test_lrange_on_nonexistent_key_returns_empty_array() {
    ...
    let resp = client
        .send_command(&["LRANGE", "never_created", "0", "-1"])
        .unwrap();
    assert_eq!(resp, "");
}
...
#[test]
fn test_two_clients_share_the_same_list() {
    ...
    writer
        .send_command(&["RPUSH", "shared", "first", "second", "third"])
        .unwrap();

    let resp = reader
        .send_command(&["LRANGE", "shared", "0", "-1"])
        .unwrap();
    assert_eq!(resp, "first,second,third");

    // The reader can also push, and the writer sees it.
    reader.send_command(&["LPUSH", "shared", "zeroth"]).unwrap();
    let resp = writer
        .send_command(&["LRANGE", "shared", "0", "-1"])
        .unwrap();
